...
check "bob-profil existiert"  "$([ -f plugins/bob/skills/bob-profil/SKILL.md ] && echo yes || echo no)" "yes"
check "bob-score kennt to_rescore" "$(grep -c 'to_rescore' plugins/bob/skills/bob-score/SKILL.md | awk '{print ($1>0)?"yes":"no"}')" "yes"
check "bob-scan kennt spar_modus"  "$(grep -c 'spar_modus' plugins/bob/skills/bob-scan/SKILL.md | awk '{print ($1>0)?"yes":"no"}')" "yes"
